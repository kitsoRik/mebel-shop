...
interface AddProps {
	add: true;
	addAction: (bed: Bed, photos: File[]) => any;
	afterAdded: () => any;
}

interface EditProps {
	edit: true;
	initialValues: Bed & { photos: string[] };
	editAction: (bed: Bed, photos: string[], removedPhotos: string[]) => any;
	afterEdited: () => any;
}

const BedsForm = (props: AddProps | EditProps) => {
	const [form] = useForm();
